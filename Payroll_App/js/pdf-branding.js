...
const PDFBranding = {
    ...
    addPayslipPage: function(doc, employee, period, calc, details, company) {
        ...
        if (details.logo_data) {
            try { doc.addImage(details.logo_data, 'PNG', margin, y, 35, 18); } catch(e) {}
            var textX = margin + 40;
        } else {
            var textX = margin;
        }
        ...
        doc.text(details.company_name || company.name || 'Company', textX, y + 8);

        doc.setFontSize(8);
        doc.setTextColor(120, 120, 120);
        if (details.address_line1) doc.text(details.address_line1, textX, y + 14);
        if (details.phone) doc.text('Tel: ' + details.phone + (details.email ? '  |  Email: ' + details.email : ''), textX, y + 19);
        ...
        doc.text('PAYSLIP', margin + 3, y + 7);

        // Period
        doc.setFontSize(10);
        var periodLabel = this.formatPeriod(period);
        doc.text(periodLabel, pageWidth - margin - doc.getTextWidth(periodLabel), y + 7);
        ...
        doc.text('Amount', pageWidth - margin - 20, y + 5);
        ...
        // Total Deductions
        var totalDed = (calc.paye || 0) + (calc.uif || 0) + (calc.totalDeductions || 0);
        doc.line(margin + 3, y, pageWidth - margin - 3, y);
        y += 5;
        doc.setFont(undefined, 'bold');
        ...
        doc.text(netStr, pageWidth - margin - 5 - doc.getTextWidth(netStr), y + 10);

        // ---- Footer ----
        doc.setFontSize(7);
        doc.setTextColor(150, 150, 150);
        doc.setFont(undefined, 'normal');
        doc.text('Generated: ' + new Date().toLocaleDateString() + ' ' + new Date().toLocaleTimeString(), margin, 280);
        doc.text('This is a system-generated document.', margin, 284);
